...
magentaNormalColor()
{
    return cv::Scalar(246, 23, 247);
}

cv::Scalar
orangeColor()
{
    return cv::Scalar(0, 117, 255);
}
...
blueNormalColor()
{
    return cv::Scalar(214, 131, 66);
}

cv::Scalar
blueLightColor()
{
    return cv::Scalar(243, 176, 108);
}

cv::Scalar
blueGrayColor()
{
    return cv::Scalar(141, 108, 77);
}
...
MergeLinesMax(std::vector<LineSegment> resLinesReal,
              ...
              const cv::Rect& box,
              const bool& useBounding)
{
    if (!MergeLinesOnce(resLinesReal, maxDegree, maxDistance, minCollinearLength, clusteredLines, box, useBounding)) {
        return false;
    }
    ...
    do {
        ...
        if (!MergeLinesOnce(before, maxDegree, maxDistance, minCollinearLength, clusteredLines, box, useBounding)) {
            return false;
        }
    ...
    for (size_t i = 0; i < resLinesReal.size(); i++) {
        ...
        for (size_t j = 0; j < clusteredLines.size(); j++) {
            ...
            if (parallelOK) {
                ...
                bool collinearOK = (diffAngleMid1 < maxDegree / 2) && (diffAngleMid2 < maxDegree / 2) && (lineLength1 > minCollinearLength) && (lineLength2 > minCollinearLength) &&
                                   (dist3D_Segment_to_Segment(resLinesReal[i], clusteredLines[j]) < maxDistance * 5);
                if (collinearOK) {
                    index = j;
                    break;
                } else {
...
{
    return getOnGlobalCoordinate(cv::Point3d(robot_pos.x, robot_pos.y, robot_pos.z), x, y);
}

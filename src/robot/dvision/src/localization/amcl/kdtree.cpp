...
void KdTree::InsertPose(Pose pose, double weight) {
    int key[3];
...
KdTreeNode* KdTree::InsertNode(KdTreeNode *parent, KdTreeNode *node, int key[], double value) {
    if(node == NULL) {
...
KdTreeNode* KdTree::FindNode(KdTreeNode *node, int key[]) {
    if(node->leaf) {
        if(node->keyEqual(key)) {
            return node;
        }
...
// D ... DFS ???
void KdTree::ClusterNode(KdTreeNode *node, int depth) {
    int nkey[3];
...
void KdTree::PrintNode(KdTreeNode* node) {
    if(node->leaf) {
